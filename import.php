    ...
    public function totalization()
    {
        ...
        for ($i = 0; $i < $sheetCount; $i++) {
            ...
            for ($j = 0; $j < count($valueAll[$i]); $j++) {
                ...
                if (!empty($square) && $threeOClockFlag == false) {
        ...
        $ingredientGram = array(); //まとめ用
        $afternoonTeaGram = array(); // まとめ用
        // 日数分ループ
        ...
        // ダウンロードリンクを生成
        $resultFilePath = "/home/kskxfree/kskxfree.wp.xdomain.jp/public_html/excel/result/result.xlsx";
        $downloadLink = "<a href='export.php?file=" . urlencode($resultFilePath) . "'>ダウンロード</a>";
        echo $downloadLink;
    }
}
